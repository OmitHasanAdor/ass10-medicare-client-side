...
import { auth } from "@/lib/auth"; // Path to your Better Auth config file
...
const DoctorDetailsPage = async ({ params }) => {
  ...
  // If no doctor is found with this ID in the database
  if (!doctor) {
    notFound();
  }

  // Retrieve current logged-in user session using Better Auth
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Left Side: Doctor Profile & Details (Spans 2 columns) */}
        ...
        {/* Right Side: Appointment Form & Payment Gateway (Spans 1 column) */}
        <div className="bg-white rounded-2xl shadow-md p-6 border h-fit sticky top-6 space-y-5">
          ...
          <form action={`${process.env.NEXT_PUBLIC_SERVER_URL}/api/create-checkout-session`} method="POST" className="space-y-4">
            {/* Hidden Fields (Required for payment and metadata processing) */}
            ...
            {/* Sending current user email obtained from Better Auth to the backend as hidden field */}
